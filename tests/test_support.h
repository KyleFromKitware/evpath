...
extern int checksum_simple_record(simple_rec_ptr event, attr_list attrs, 
				  int quiet);
extern void test_fork_child(EVstone * stones, char **receiver_contact);
extern int wait_for_children();
extern int quiet;
extern int be_test_receiver(int argc, char **argv);
extern int be_test_sender(int argc, char **argv);
