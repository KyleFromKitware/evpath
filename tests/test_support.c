...
extern void
test_fork_child(EVstone * stones, char **receiver_contact)
{
    char *args[20];
    int i = 0;
	int stone_index = 0;
    ...
    args[i++] = "-c";
	while(receiver_contact[stone_index] != NULL) 
	{
		size_t approximate_size_of_string = strlen(receiver_contact[stone_index]) + 5;
		args[i] = (char *) malloc(sizeof(char) * approximate_size_of_string);
    	sprintf(args[i++], "%d:%s", stones[stone_index], receiver_contact[stone_index]);
		stone_index++;
	}
...
delay_fork_wrapper(CManager cm, void *client_data)
{
    ...
    (void) cm;
    test_fork_child(str->list, str->master_contact);
    free(str);
	*/
}
...
main(int argc, char **argv)
{
	...
	//Okay so we are trying to strip out arguments, but we do a poor job for the child without a hack so...
	if(argv[0][0] == '-')
	{
		argv++;
		argc--;
	...
	*/
    ...
    if (!regression_receiver) {
		ret = be_test_sender(argc, argv);
    } else {
		ret = be_test_receiver(argc, argv);
    }
    free(subproc_args);
    return ret;
}
